...
const _6nbvmZLRvB4eEFi4155cM_0Pf8pBrXZAoi3IeN4Uo = defineNitroPlugin((nitroApp) => {
  ...
  nitroApp.hooks.hook("error", (event, { body }) => {
    consola$1.error(event.message, event.message, body);
  });
  nitroApp.hooks.hook("afterResponse", (event) => {
    consola$1.success(`${event.method} ${event.path}`);
  });
});
...
function createErrorResponse(message = "Request failed.", code = ResultConstants.ERROR, data = null) {
  return {
    code,
    data,
    message,
...
const login_post = eventHandler(async (event) => {
  ...
  const matchedUser = fakeUserList.find(
    (user2) => user2.username === username && password === user2.extraInfo?.password
  );
  if (!matchedUser) {
...
function getFakeUserByToken(authorizationToken) {
...
const logout = eventHandler((event) => {
  try {
    ...
    return createSuccessResponse(void 0, "Token has been destroyed");
  } catch (error) {
...
const permissionCode = eventHandler((event) => {
  try {
    const authorizationToken = extractAuthorizationToken(event);
...
const permissionCode$1 = /*#__PURE__*/Object.freeze({
  __proto__: null,
  default: permissionCode
});

const dataOverview = eventHandler(() => {
  function generateLineData(name) {
    const count = getRandomNumber(20, 2e3);
    const value = Array.from({ length: 8 }).fill(0).map(() => getRandomNumber(800, 4e3));
    return {
      name,
      count,
      value
...
const dataOverview$1 = /*#__PURE__*/Object.freeze({
  __proto__: null,
  default: dataOverview
});

const userAnalysisData = eventHandler(async (event) => {
  try {
    ...
    const getLineData = () => {
      ...
      const data = {
        name: quota,
        value: Array.from({ length: 12 }).fill(0).map(() => getRandomNumber(1e3, 3e3))
      };
      return { xAxis, data };
    };
    return createSuccessResponse({
      count: getRandomNumber(1e3, 3e3),
...
const userAnalysisData$1 = /*#__PURE__*/Object.freeze({
  __proto__: null,
  default: userAnalysisData
});

const index = defineEventHandler(() => {
  return { hello: "@celeris/admin-api" };
});
...
const info = eventHandler((event) => {
  try {
    ...
    return createSuccessResponse({ ...user, extraInfo: void 0 });
  } catch (error) {
    return createErrorResponse(getErrorMessage(error));
  }
});
