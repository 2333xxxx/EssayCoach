...
export function fetchUsers(params?: { page?: number; limit?: number; search?: string; role?: string; status?: string }) {
...
export function deactivateUser(id: number) {
  return request<any>({ url: `/users/${id}`, method: 'patch', data: { status: 'inactive' } });
}
